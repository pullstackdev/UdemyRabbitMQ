...
namespace UdemyRabbitMQWeb.Watermark.Controllers
{
    public class ProductsController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("Id,Name,Price,Stock,ImageName")] Product product, IFormFile ImageFile) //ImageFile'ı name'den yakalar
        {
            ...
            if (ImageFile is { Length:>0})
            {
                ...
                await ImageFile.CopyToAsync(stream); //kopyaladı artık event fırlat
                _rabbitMQPublisher.Publish(new ProductImageCreatedEvent(){ ImageName = randomImageName }); //eventi datası ile fırlattık
